  ...
  return (
    <div className="bg-surface border border-line rounded-lg p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-ink-muted mb-1">Agency type</label>
        ...
        <div>
          <label className="block text-xs font-medium text-ink-muted mb-1">Contact email</label>
        ...
        <div>
          <label className="block text-xs font-medium text-ink-muted mb-1">Contact role</label>
          <input value={draft.contactRole} onChange={(e) => set('contactRole', e.target.value)} placeholder="A&R, Sync Supervisor…" className={INPUT} />
        </div>
        <div>
          <label className="block text-xs font-medium text-ink-muted mb-1">Confirm via</label>
        ...
        <div>
          <label className="block text-xs font-medium text-ink-muted mb-1">Initial status</label>
        ...
        <div className="col-span-2">
          <label className="block text-xs font-medium text-ink-muted mb-1">Pitch draft</label>
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-ink-subtle mb-1">Agency type</label>
          <input value={draft.agencyType} onChange={(e) => set('agencyType', e.target.value)} className={INPUT} />
        </div>
        <div>
          <label className="block text-xs font-medium text-ink-subtle mb-1">Contact email</label>
          <input type="email" value={draft.contactEmail} onChange={(e) => set('contactEmail', e.target.value)} className={INPUT} />
        </div>
        <div>
          <label className="block text-xs font-medium text-ink-subtle mb-1">Contact role</label>
          <input value={draft.contactRole} onChange={(e) => set('contactRole', e.target.value)} className={INPUT} />
        </div>
        <div className="col-span-2">
          <label className="block text-xs font-medium text-ink-subtle mb-1">Confirm via</label>
        ...
        <div className="col-span-2">
          <label className="block text-xs font-medium text-ink-subtle mb-1">Pitch draft</label>
...
function SyncDetail({ target, onEdit }: { target: SyncTarget; onEdit: () => void }) {
  return (
    <div className="space-y-4 max-w-2xl">
      {target.notes && (
        <div>
          <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1.5">Notes</p>
          <p className="text-sm text-ink-muted leading-relaxed">{target.notes}</p>
        </div>
      )}
      {target.pitchDraft && (
        <div>
          <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1.5">Pitch Draft</p>
          <p className="text-sm text-ink-muted leading-relaxed whitespace-pre-wrap bg-surface rounded-md border border-line p-3">
            {target.pitchDraft}
          </p>
        </div>
      )}
      {target.confirmationMethod && (
        <span className="inline-block bg-surface px-2.5 py-1 rounded-md border border-line text-xs text-ink-muted">
          Confirm via {target.confirmationMethod}
        </span>
      )}
      {!target.notes && !target.pitchDraft && !target.confirmationMethod && (
        <p className="text-xs text-ink-subtle">No additional details.</p>
      )}
      <button onClick={onEdit} className="text-xs px-3 py-1.5 rounded-md border border-line-strong text-ink-muted hover:bg-surface-muted transition-colors">
        Edit details
      </button>
    </div>
  )
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="bg-surface border border-line rounded-lg divide-y divide-line">
            ...
            return (
              <div key={target.id}>
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${isOpen ? 'bg-library-soft/40' : 'hover:bg-surface-muted'}`}
                >
                  <Chevron open={isOpen} />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-ink truncate">{target.name}</p>
                    <div className="flex gap-2 mt-0.5 flex-wrap">
                      {target.agencyType && <span className="text-xs text-ink-muted capitalize">{target.agencyType}</span>}
                      {target.contactEmail && <span className="text-xs text-ink-subtle">{target.contactEmail}</span>}
                      {target.contactRole && <span className="text-xs text-ink-subtle">· {target.contactRole}</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {target.status === 'draft_ready' && (
                      <button disabled={isPatching}
                        ...
                        className="text-xs px-2.5 py-1 rounded-md bg-scheduled-soft text-scheduled hover:bg-scheduled-soft disabled:opacity-40 transition-colors">
                        Mark Pitched
                      </button>
                    )}
                ...
                {isOpen && (
                  <div className="px-6 pb-5 pt-3 bg-library-soft/40 border-t border-library">
                    {isEditing ? (
                      <EditSyncPanel
                        target={target}
          ...
          {data.length === 0 && (
            <p className="px-4 py-12 text-center text-ink-subtle text-sm">No sync targets found.</p>
          )}
        </div>
      )}

      {!isLoading && <p className="text-xs text-ink-subtle">{data.length} targets</p>}
    </div>
  )
}
