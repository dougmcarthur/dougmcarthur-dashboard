    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="bg-surface border border-line rounded-lg divide-y divide-line">
              ...
              <div key={draft.id}>
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${
                    isOpen ? 'bg-pending-soft/40' : 'hover:bg-surface-muted'
                  }`}
                >
                  <Chevron open={isOpen} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-ink truncate">{draft.title}</p>
                    <p className="text-xs text-ink-muted mt-0.5">{draft.month}</p>
                  </div>
                ...
                {isOpen && (
                  <div className="px-6 pb-5 pt-3 bg-pending-soft/40 border-t border-pending">
          ...
          {data.length === 0 && (
            <p className="px-4 py-12 text-center text-ink-subtle text-sm">No promo drafts.</p>
          )}
        </div>
      )}

      {!isLoading && (
        <p className="text-xs text-ink-subtle">{data.length} drafts</p>
      )}
    </div>
  )
}
